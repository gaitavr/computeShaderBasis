...
public class GpuTest : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        if (GUI.Button(new Rect(250, 50, 200, 100), "GPU"))
        {
            Regenerate();
        }
    }
    ...
    private void Regenerate()
    {
        ...
        _shader.SetInt("Iterations", _iterations);
        _shader.SetInt("Width", _cubesContainer.Width);
        _shader.SetInt("Height", _cubesContainer.Height);
        
        var threadGroups = (int)(BufferSize / _threadGroupSize);
        _shader.Dispatch(_kernelIndex, threadGroups, 1, 
            1);
